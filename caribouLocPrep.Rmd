...
The init event schedules the data processing events. "downloadData" harmonizes the downloaded data from all jurisdictions requested 
and applies advanced filters for removing unnatural speed between points, distance traveled, and round trips. 
"createFullExtent" generates a buffered/convex hull study area of all the points requested, used in landscape preparation.
"createJurisdictionExtents" generates a buffered study area for each jurisdiction, this is used for global simulations.
...
studyArea_juris is saved to a google drive folder for use in the SSUD module. 

### Module outputs

Description of the module outputs (Table \@ref(tab:moduleOutputs-caribouLocPrep)).

caribouLoc is a data.table of cleaned and harmonized points with advanced filters applied.
studyAreaCaribou is the buffered/convexhull of the full dataset requested
studyArea_juris is a collection of polygons from buffered points for each jurisdiction requested
